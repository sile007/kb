...
```bash
# sidecar-test-app-conjur-map.yaml
---
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  conjur-map: |-   
    username: sidecar-test-app-secrets/username
    password: sidecar-test-app-secrets/password
```

```bash
kubectl apply -f sidecar-test-app-conjur-map.yaml -n sidecar-test-app
```

Check the created secret with
```bash
kubectl get secrets -n sidecar-test-app
```
